...
if LIKELIHOOD.InUse==LIKELIHOOD.Gaussian
  ...
  % I had to modify Tipping's original code because sometimes U matrix 
  % turned out not to be positive definite so chol function didn't work
  AMat   = PHI'*PHI*beta + diag(Alpha);
  [~, p] = chol(AMat); 
  if p ~= 0
      AMat = nearestSPD(AMat);
  end
  U		 = chol(AMat);
  Ui	 = inv(U);
  SIGMA	 = Ui * Ui';
